...
namespace Kixeye.Jabber.Connection
{
    ...
    public class DiscoNode : JIDNode, IEnumerable
    {
        ...
        private Kixeye.Jabber.Protocol.X.Data[] m_extensions;
        ...
        /// </summary>
        public Kixeye.Jabber.Protocol.X.Data[] Extensions
        {
            get
            {
                return m_extensions;
    ...
    public class DiscoManager : StreamComponent, IEnumerable
    {
        ...
        public DiscoManager()
        {
            this.OnStreamChanged +=new Kixeye.Bedrock.ObjectHandler(DiscoManager_OnStreamChanged);
        }
        ...
        private void DiscoManager_OnStreamChanged(object sender)
        {
            if (m_stream == null)
                return;
            m_stream.OnAuthenticate += new Kixeye.Bedrock.ObjectHandler(m_client_OnAuthenticate);
            m_stream.OnDisconnect += new Kixeye.Bedrock.ObjectHandler(m_stream_OnDisconnect);
        ...
        private void RequestInfo(DiscoNode node)
        {
            lock (node)
            {
                if (!node.PendingInfo)
                {
                    ...
                    BeginIQ(iq, new Kixeye.Jabber.Connection.IqCB(GotInfo), node);
                }
            }
        }
                ...
                if (dn == m_root)
                {
                    ...
                    if (err != null)
                    {
                            ...
                            (cond == Error.SERVICE_UNAVAILABLE))
                        {
                            ...
                            BeginIQ(aiq, new Kixeye.Jabber.Connection.IqCB(GotAgents), m_root);
                            return;
                        }
                    }
                }
